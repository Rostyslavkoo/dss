...
class SidonNetwork(AbstractNetwork):
    """Network implementation for Sidon's DFS algorithm testing."""
    NUMBER_OF_NODES = 8
    ...
    def __init__(self) -> None:
        ...
        for node_id in ids:
            self.nodes.append(SidonAlgorithmNode(node_id, self.edges[node_id], ids))
        super().__init__(self.nodes)

    def __get_edges(self, ids: List[uuid.UUID]) -> Dict[uuid.UUID, List[uuid.UUID]]:
        """Define network topology for DFS traversal."""
        self.edges = {
            ids[0]: [ids[1], ids[2]],
