...
class AverbukNetwork(AbstractNetwork):
    """Network implementation for Averbukh's BFS algorithm testing."""
    NUMBER_OF_NODES = 8
    ...
    def __init__(self) -> None:
        ...
        for node_id in ids:
            self.nodes.append(AverbukAlgorithmNode(node_id, self.edges[node_id], ids))
        super().__init__(self.nodes)

    def __get_edges(self, ids: List[uuid.UUID]) -> Dict[uuid.UUID, List[uuid.UUID]]:
        """Define network topology as a tree for BFS."""
        self.edges = {
            ids[0]: [ids[1], ids[2]],
